...
class SchedulerTests(unittest.TestCase):
    def test_topological_order_prioritizes_newly_ready_tasks(self):
        tasks = {
            "a": task(),
            "b": task(["a"]),
            "c": task(["a"]),
    ...
    def test_execution_batches_skip_conflicts_and_fill_capacity(self):
        tasks = {
            "a": task(group="x"),
            "b": task(group="x"),
            "c": task(group="y"),
    ...
    def test_malformed_schemas_are_rejected_with_value_error(self):
        invalid_tasks = [
            None,
            [],
            {1: task()},
        ...
        for tasks in invalid_tasks:
            for operation in (
                ...
                with self.subTest(tasks=tasks, operation=operation):
                    with self.assertRaises(ValueError):
                        operation(tasks)

    def test_invalid_capacity_is_rejected_even_for_empty_tasks(self):
    ...
    def test_weighted_critical_path_ignores_groups(self):
        tasks = {
            "a": task(duration=2, group="one"),
            "b": task(["a"], duration=3, group="two"),
            "c": task(["a"], duration=5),
            "d": task(["b", "c"], duration=4),
            "z": task(duration=20, group="one"),
        }
        self.assertEqual(critical_path_length(tasks), 20)
        tasks.pop("z")
        self.assertEqual(critical_path_length(tasks), 11)

    def test_empty_tasks_and_capacity(self):
