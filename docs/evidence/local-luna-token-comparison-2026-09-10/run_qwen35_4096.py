...
dest = root/'qwen35-local-4096'
dest.mkdir(exist_ok=False)
body = json.loads((root/'local/request.json').read_text())
body['model'] = 'qwen35-mtplx-quality'
...
(dest/'frozen-inputs.json').write_text(json.dumps({n:hashlib.sha256((root/n).read_bytes()).hexdigest() for n in ['contract.txt','acceptance.py','local/request.json','run_qwen35_4096.py']},indent=2)+'\n')
started = time.monotonic()
request = urllib.request.Request('http://127.0.0.1:18094/v1/chat/completions',data=json.dumps(body).encode(),headers={'Content-Type':'application/json'})
