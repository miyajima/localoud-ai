...
計画・レビューは、コピー後にLocaloudで ⌘⇧V（Windows/LinuxはCtrl+Shift+V）を押すと取り込めます。ChatGPTで計画するモードだけで有効です。取り込み後は対象タスクの概要へ移動し、計画は実行、修正指示は修正・再検証、合格は完了へ進みます。実行中・アーカイブ済みのタスクでは取り込みません。クリップボードの常時監視は行いません。

上部の工程欄では、次に使うボタンを先頭に表示します。レビュー依頼をコピーすると「結果を取り込んで次へ」が先頭になり、成果物の版が更新されると新しい版の「レビュー依頼をコピー」に戻ります。
