...
「操作」は **相談・対象実装／計画して実装／選択モデルで計画を相談** を使い分けます。「モデル」は接続先が返すモデル名から選択します。ローカルモデルには「ローカル」と補足します。モデル一覧の更新ボタンはモデル情報を取得するだけで、タスクの推論を始めません。

- 明示したモデルはそのまま使用します。利用不可・モデル不一致なら停止し、別モデルへ置換しません。
- 「自動」のときだけルーティングします。設計・認証など計画が必要な依頼では、実装を始めず計画作成を案内します。
- 対象ファイルを明示した依頼だけが、専用worktree上の実装と別セッションレビューへ進みます。対象未指定のCodex/API会話は読み取り専用です。大きい、または範囲が未確定の実装は「計画して実装」で開始します。
- ローカル編集はモデルを差し替えても1〜2個の指定ファイルへの小規模な置換に制限されます。モデルを大きくしても自動的に権限は増えません。旧来の親checkoutへの直接置換は無効です。
...
モデル一覧はprofile単位で表示します。同じモデルIDでも「profile名 / model名」で区別されます。複数のOpenAI互換Chat、OpenAI Responses、Anthropic Messagesを登録できます。ChatGPT Webの会話とAPI profileは別の接続・課金境界です。設定と制約は [API provider profiles](API_PROVIDERS.md) を参照してください。
...
「自動」では [エージェントの振り分け設定](AUTO_ROUTING.md) に保存したエージェントとモデル・Reasoning を使います。エージェントは自由に追加・削除でき、判定、難易度1〜5、計画、レビュー、代替実行へ割り当てられます。
