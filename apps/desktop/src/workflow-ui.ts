...
export type Flow = {project:string;title:string;status:string;kind:'manifest'|'direct'|'legacy';hasTask:boolean;planningRequested:boolean;reviewRequested:boolean;iteration?:number;id?:string;version?:string|null;busy:boolean;importing:boolean;stopping:boolean;writing?:boolean};
...
export function flowMarkup(f:Flow){
 ...
 if(!f.project){next='プロジェクトを選ぶか、フォルダを登録して作業を始めます。';}
  ...
  next=!f.hasTask?'下の入力欄に、やりたいことを書いて送信してください。':halted?'作業が中断しています。状態を確認して再開してください。':index===2?'実行が終了しました。変更と結果を確認してください。レビューの合格は別途確認が必要です。':'依頼した内容を実行しています。進捗は概要、詳細はログで確認できます。';
  if(halted)actions=action('resume','再開・状態を確認',true);
  else if(f.hasTask&&index===1)actions=action('stop',f.stopping?'停止中…':'停止',false,f.stopping);
 }else if(!f.hasTask){
  next='左のChatGPTで計画を相談し、確定した計画JSONを取り込んで実行します。';
  actions=action('import',f.importing?'取り込み中…':'計画を取り込んで実行',true);
