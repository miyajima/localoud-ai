...
export type AutoSettings = {classifier:ModelTarget;levels:{level:number;target:ModelTarget}[];fallback:ModelTarget|null;planner_default:ModelTarget|null;reviewer_default:ModelTarget|null;confirm_before_run:boolean};
...
type Dependencies = {call:Call;models:()=>ModelChoice[];refresh:()=>Promise<void>;busy:()=>boolean;run:(preview:AutoPreview)=>void;plan:(preview:AutoPreview)=>void;notice:(text:string)=>void};
...
export async function openAutoSettings(){
  ...
  try{
    ...
    write('planner-default',config.planner_default||config.levels.find(row=>row.level===5)?.target||null);
    write('reviewer-default',config.reviewer_default||config.planner_default||config.levels.find(row=>row.level===5)?.target||null);
...
export function setupAutoRouting(value:Dependencies){
  deps=value;
  document.body.insertAdjacentHTML('beforeend',`<dialog id="auto-settings-dialog" class="auto-dialog"><form id="auto-settings-form"><h2>おまかせの振り分け設定</h2><p>依頼文と対象ファイル名から難易度を推定し、設定したモデルへ振り分けます。モデルは複数の段階に割り当てられます。</p><fieldset id="auto-settings-fields"><legend class="sr-only">振り分け設定</legend><h3>難易度を判定するモデル</h3>${controls('classifier')}<h3>難易度ごとの実行モデル</h3><div class="auto-table-wrap"><table class="auto-table"><thead><tr><th>難易度・目安</th><th>モデルと Reasoning</th></tr></thead><tbody>${levels.map(([title,description],i)=>`<tr><th scope="row">${i+1} · ${title}<small>${description}</small></th><td>${controls(`level-${i+1}`)}</td></tr>`).join('')}</tbody></table></div><h3>計画とレビューの既定値</h3>${controls('planner-default')}${controls('reviewer-default')}<h3>判定できない・モデルが利用できない場合</h3><label for="auto-fallback-mode">扱い</label><select id="auto-fallback-mode"><option value="stop">停止して自分で選ぶ</option><option value="model">指定した代替モデルを使う</option></select><div id="auto-fallback-controls">${controls('fallback')}</div><label class="auto-check"><input id="auto-confirm" type="checkbox">実行前に振り分け結果を確認する</label><p class="muted">ローカル編集の範囲や、計画が必要な依頼の扱いは維持されます。計画・レビューの既定値はセッションごとに上書きできます。保存済みの指定は優先します。</p><button type="submit" class="primary">保存する</button></fieldset><p id="auto-settings-error" role="alert"></p><div class="dialog-actions"><button id="auto-settings-close" type="button">閉じる</button></div></form></dialog><dialog id="auto-preview" class="auto-dialog"><h2>進め方の確認</h2><p>難易度とモデルを変更できます。変更しても依頼内容の制限は維持されます。</p><fieldset id="route-fields"><label for="route-level">難易度</label><select id="route-level"><option value="" disabled>未判定</option>${levels.map(([title],i)=>`<option value="${i+1}">${i+1} · ${title}</option>`).join('')}</select>${controls('route')}</fieldset><p id="route-state" role="status"></p><details open><summary>判定理由</summary><p id="route-reason" class="preserve-lines"></p></details><p id="route-error" role="alert"></p><div class="dialog-actions"><button id="route-close">閉じる</button><button id="route-plan" hidden>計画を作成</button><button id="route-run" class="primary">この設定で実行</button></div></dialog>`);
  for(const prefix of ['classifier','fallback','planner-default','reviewer-default',...levels.map((_,i)=>`level-${i+1}`)])select(`${prefix}-model`).addEventListener('change',()=>fillReasoningSelect(select(`${prefix}-reasoning`),deps.models().find(m=>m.key===select(`${prefix}-model`).value)));
  ...
  document.getElementById('auto-settings-form')!.addEventListener('submit',async event=>{
    ...
    try{
      const config:AutoSettings={classifier:read('classifier'),levels:levels.map((_,i)=>({level:i+1,target:read(`level-${i+1}`)})),fallback:select('auto-fallback-mode').value==='model'?read('fallback'):null,planner_default:read('planner-default'),reviewer_default:read('reviewer-default'),confirm_before_run:(document.getElementById('auto-confirm') as HTMLInputElement).checked};
      saving=true;fields.disabled=true;document.getElementById('auto-settings-error')!.textContent='';
      await deps.call('set_auto_settings',{config});dialog('auto-settings-dialog').close();deps.notice('おまかせの振り分け設定を保存しました。次の判定から反映します。');
